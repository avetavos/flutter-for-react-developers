...
<ReactFlutter react={ciReact} flutter={ciFlutter} reactTitle="React / GitHub Actions" flutterTitle="Flutter / GitHub Actions" />
...
- **Android projects use Flutter's built-in Kotlin.** Since Flutter 3.44, `flutter create` no longer wires a separate Kotlin Gradle plugin version into your project — the SDK supplies it. An older project still carrying its own Kotlin version can keep building, but `flutter doctor` and the build logs will point you at the migration when it stops matching.
